...
async function restSignUp(req, res) {
  try {
    ...
    let Zid = "";
    let zones = await zone.findAll({attributes:['id','coordinates']});
    
    const point = turf.point([lng, lat]);

// Iterate over each zone to check if the point lies within its polygon
// for (const zone of zones) {
//     const coordinates = zone.coordinates.coordinates[0];
//     const polygon = turf.polygon([coordinates]);
    
//     if (turf.booleanPointInPolygon(point, polygon)) {
//         console.log("mil gya hai ");
//         console.log("***************************************************************************")
//         Zid = zone.id;
//         break; // Exit the loop once a zone is found
//     }
// }

    // if(Zid == ""){
    //     let response = ApiResponse("0","Location is not exist in any Zone","Error",{});
...
//  return res.json(Zid);   
    ...
    newUser.email = email;
    newUser.countryCode = code;
    newUser.phoneNum = businessPhone;
    ...
    newUser
      ...
      .then(async (data) => {
        ...
        const ratePerHour = await defaultValues.findOne({
          where: {
            name: "ratePerHour",
          },
        });
        ...
        (newRest.phoneNum = businessPhone),
          ...
          (newRest.ratePerHour = ratePerHour.value),
        ...
        newRest
          ...
          .then(async (dat) => {
            ...
            config.delivery = 1;
            config.takeAway = 1;
            config.tableBooking = 1;
            config.stampCard = 1;
            config.cod = 1;
            config.euro = 1;
            config.print = 1;
            config.deliveryByYourDrivers = 1;
            
            config.isOpen_pickupOrders = 1;
            config.isOpen_deliveryOrders = 1;
            config.isOpen_schedule_pickupOrders = 1;
            config.isOpen_schedule_deliveryOrders = 1;
            
            config.isClose_schedule_pickupOrders = 1;
            config.isClose_schedule_deliveryOrders = 1;
            
            config.isClose_schedule_pickupOrders = 1;
            config.temporaryClose_schedule_pickupOrders = 1;
            config.temporaryClose_schedule_deliveryOrders = 1;
            
            config.isRushMode_pickupOrders = 1;
            config.isRushMode_deliveryOrders = 1;
            config.isRushMode_schedule_pickupOrders = 1;
            config.isRushMode_schedule_deliveryOrders = 1;
            config.selfDelivery = 1;
            
            config.status = 1;
            // config.restaurantId  = 1;
            config.restaurantId = dat.id;
            await config.save();
...
async function addDirector(req,res){
    ...
    const { firstName,lastName,dob,streetAddress,zip,city,country,bankName,accountHolderName,accountNo,IBAN,swiftCode,bankAddress,bankCountry,restaurantId} = req.body;
    
    let check = await director.findOne({where:{restaurantId : restaurantId}});
    if(check){
...
// });
 ...
 const outgoing_orders = await order.findAll({
  ...
  let config = await configuration.findOne({where:{restaurantId : userData.restaurants[0].id},attributes:['selfDelivery']});
  const data = {
    incomming_orders,
    ready_orders: list,
...
async function homeData(restId) {
  try {
    ...
    const incomming_orders = await order.findAll({
      ...
      where: [
        //   {paymentConfirmed:true},{paymentRecieved:true},
        {
          restaurantId: restaurantId,
        },
        {
  ...
  if (orderData) {
    ...
    orderData
      ...
      .then(async (dat) => {
        ...
        sendEvent(orderData?.restaurant?.user?.ip,eventDataForRetailer)
    })
    .catch((error)=>{
        console.log(error)
    })
...
async function rejectOrder(req, res) {
  ...
  const orderData = await order.findOne({
    ...
    include: [{
      model: user,
      attributes: ["id", "deviceToken","ip"],
    },{model:restaurant,attributes:['id'],include:{model:user,attributes:['ip']}}],
  });
      ...
      .then((dd) => {
        ...
        history
          ...
          .then(async(his) => {
            ...
            sendEvent(orderData?.restaurant?.user?.ip, restEventData);
  ...
  let outObj = {
    ...
    driverDetails: orderData.driverId
      ? {
          ...
          image: orderData.DriverId.driverDetails[0]?.profilePhoto ? orderData.DriverId.driverDetails[0]?.profilePhoto : "",
...
async function readyForPickup(req, res) {
  ...
  const orderData = await order.findOne({
    ...
    include: [
      {
        model: restaurant,include:{model:user,attributes:['ip']}
      },
      {
        model: deliveryType,
      },
        ...
        history
          ...
          .then(async (dd) => {
            ...
            if (orderData.deliveryTypeId == "2") {
              ...
              //send to driver
              sendEvent(orderData?.DriverId?.ip, eventData);
              sendEvent(orderData?.user?.ip, eventData);
              homeData(orderData.restaurantId).then((dat)=>{
                    let restData = {
                        type: "readyForPickup",
                    ...
                    sendEvent(orderData?.restaurant?.user?.ip, restData);
              })
        ...
        include: [
          {
            ...
            required: false, // This allows products without addons to be included
            include: {
              model: addOn,
              include: {
                model: collectionAddons,
                attributes: ['id', 'collectionId'],
                required: false // This allows products without collection addons to be included
              },
          ...
          },
          {
            ...
            where: { status: true },
            required: false, // This allows products without collections to be included
            include: [
              {
                model: collection,
                where: { status: true },
                attributes: ["id", "title", "minAllowed", "maxAllowed", "status"],
                required: false,
                include: [
                  {
                    model: collectionAddons,
...
async function addAddOns(req, res) {
  ...
  if (minAllowed > maxAllowed) {
   ...
   let restData = await restaurant.findOne({where:{id:restaurantId},attributes:['businessType']})
     ...
     const check = await addOn.findOne({ where: { name: name,restaurantId : restaurantId,status:true } });
     if (check) {
       const response = ApiResponse("0", "Already exists", "", {});
       return res.json(response);
     } 
//   else
  {
    ...
    newAddon.orderApplicationName = restData.businessType;
    ...
    newAddon
      ...
      .then(async (dat) => {
        ...
        collAdd.minAllowed = minAllowed ? minAllowed : 1 ;
        collAdd.maxAllowed = maxAllowed ? maxAllowed : 5;
        collAdd.price = price;
        collAdd.status = true;
      ...
      .then(async (dd) => {
        ...
        if (col) {
          ...
          await col.save();
            ...
            let newC = new collectionAddons();
            newC.collectionId = collectionId;
            newC.price = price;
            newC.isPaid = isPaid;
            newC.isAvaiable = isAvaiable;
            newC.maxAllowed = maxAllowed;
            newC.addOnId   = dd.id;
            newC.price = price;
            newC.minAllowed = minAllowed;
            await newC.save();
        }
...
async function deleteAddonCategory(req, res) {
  const { id } = req.body;
    ...
    let data = await collection.findOne({ where: { id: id } });
    console.log(data);
    ...
    if (data) {
      ...
      // Find and destroy all related collectionAddons
      let coll = await collectionAddons.findAll({ where: { collectionId: id } });
      if (coll.length > 0) {
        for (const col of coll) {
          let add = await addOn.findOne({where:{id:col.addOnId }});
          if(add){
              add.status = false;
              await add.save();
          }
    ...
    //   // Find and destroy all related productCollections
    //   let prod = await productCollections.findAll({ where: { collectionId: id } });
    //   if (prod.length > 0) {
    ...
    //   }
      ...
      let response = ApiResponse("1", "Collection removed successfully", "", {});
      return res.json(response);
    } else {
    ...
    console.error(error);
    let response = ApiResponse("0", "An error occurred", "Error", {});
  ...
  try {
    ...
    const table = await tableBooking.findOne({
      ...
      include: [
        {
          model: restaurant,
          attributes: ["id", "businessName"],include:{model:user,attributes:['ip']}
        },
        {
          model: user,
          attributes: ["id", "deviceToken","ip"],
    ...
    if (table) {
      ...
      table
        ...
        .then(async(dat) => {
            ...
            sendEvent(table?.restaurant?.user?.ip,eventDataForRetailer)
        })
        .catch((error)=>{
            console.log(error)
        })
    ...
    if (fireBase) {
      if (fireBase.data[driver.user.id]) {
          if(driver?.user?.deviceToken){
              let tokens = formatTokens(driver?.user?.deviceToken);
              tokens.map((token)=>{
                  list.push(token)
              })
          }
...
async function changeProductStatus(req,res){
    ...
    let data = await R_PLink.findOne({where:{id:productId}});
    if(data){
        data.status = status;
         data.save()
                .then(dat => {
...
async function updateDirector(req,res){
    const { directorId,bankName,accountHolderName,accountNo,IBAN,swiftCode,bankAddress,bankCountry,streetAddress,zip,city,country } = req.body;
    let data = await director.findOne({where:{id:directorId}});
    if(data){
...
async function send_invitation(req,res){
    ...
    let type = await userType.findOne({where:{name:'driver'}});
    let driver = await user.findOne({where:{countryCode:countryCode,phoneNum:phoneNum,status:true,userTypeId : type.id}});
    if(driver){
        ...
        let response = ApiResponse("0","Sorry not found!","Error",{});
        return res.json(response);
    }
}
...
async function allCuisines(req,res){
    const { restaurantId } = req.params;
    let cc = await cuisine.findAll({where:{status:true}});
...
async function assignCuisine(req,res){
    ...
    let check = await cusineRestaurant.findOne({where:{cuisineId:cuisineId,restaurantId:restaurantId,status:true}});
    if(check){
        check.status = true;
        check.save().then(dat =>{
            let response = ApiResponse("1","Cuisine assign successfully","",{});
            return res.json(response);
        })
        .catch((error)=>{
            let response = ApiResponse("0",error.message,"Error",{});
        ...
        let dd = new cusineRestaurant();
        dd.restaurantId = restaurantId;
        dd.cuisineId = cuisineId;
        dd.status = true;
        dd.save().then(dat =>{
            let response = ApiResponse("1","Cuisine assign successfully","",{});
            return res.json(response);
        ...
        .catch((error)=>{
            let response = ApiResponse("0",errro.message,"Error",{});
            return res.json(response);
        })
    }
}

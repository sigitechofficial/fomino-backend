...
async function payrexx_design(req,res){
  ...
  let sign = buildSignature(designData, secret);
    ...
    const requestData = qs.stringify(designData, { format: "RFC3986" });


    const response = await axios.post(
      `https://api.payrexx.com/v1.0/Design/?instance=${instanceName}`,
      requestData,
      {
        headers: {
...
async function payrexx_payment(req, res) {
  ...
  try {
    ...
    const { orderId, currency, vatRate ,isCredit , paymentProvider,paymentMethod } = req.body;

    let orderData = await order.findOne({
      where: { id: orderId },
      include: [{ model: user,include:{model:Credit} },{model:restaurant,attributes:['packingFee']}],
    });
    ...
    const invoiceData = 
    {
      ...
      successRedirectUrl:"https://backend.fomino.ch/success.html",
      failedRedirectUrl:"https://backend.fomino.ch/failed.html",
    ...
    const response = await axios.post(
      `https://api.payrexx.com/v1.0/Gateway/?instance=${instanceName}`,
      requestData,
      {
        headers: {
          "Content-Type": "application/x-www-form-urlencoded",
...
async function get_all_active_payment_methods(req,res){
        let sign = buildSignature({filterCurrency: 'CHF'}, secret);
              ...
              stripe.customers
                ...
                .then((customer) => {
                  ...
                  bcrypt.hash(password, 10).then((hashedPassword) => {
                    ...
                    if (userCredit) {
                        const currentPoints = parseInt(userCredit.point);
                        const maxPoints = 18;
                        const pointsToAdd = 4;
                    
                        if (currentPoints < maxPoints) {
                            const newPoints = currentPoints + pointsToAdd > maxPoints ? maxPoints : currentPoints + pointsToAdd;
              ...
              bcrypt.hash(password, 10).then((hashedPassword) => {
                ...
                user
                  ...
                  .then(async (userData) => {
                    ...
                    if (userCredit) {
                      userCredit.point = parseFloat(userCredit.point) + 2;
                      await userCredit.save();
                      singleNotification(
                        userCredit.user.deviceToken,
                        "Got Bonus Points",
...
async function forgetPasswordRequestForRetailer(req, res) {
  ...
  try {
    ...
    if (!userData) {
      const response = ApiResponse(
        "0",
        "No Retailer exists against the provided email",
        "Please sign up first",
        {}
      );
      return res.json(response);
...
async function getRestaurantById(req, res) {
  ...
  try {
   ...
   let getConfiguration = await configuration.findOne({where:{restaurantId : restaurantId}});
    let retObj = {
...
async function orderAfterPayment(req, res) {
  ...
  try {
    ...
    // let type = await orderType.findOne({where:{name:"Group"}});
    // let groupOrder = await order.findOne({ where: { id: orderId ,orderTypeId : type.id}, include: [{model:restaurant},{model:user,include:{model:Credit}},{ model: orderCharge }, { model: address, as: "dropOffID" }] });
    // if(groupOrder){
    //     groupOrder.isLocked = true;
    //     await groupOrder.save();
    // }
    let orderData = await order.findOne({ where: { id: orderId }, include: [{model:restaurant},{model:user,include:{model:Credit}},{ model: orderCharge }, { model: address, as: "dropOffID" }] });
    ...
    const restaurantData = await restaurant.findOne({ where: { id: orderData.restaurantId, }, include: { model: user, attributes: ["deviceToken", "ip"], }, });
    const restDrivers = await restaurantDriver.findAll({ where: { restaurantId: orderData?.restaurantId, }, include: { model: user, attributes: ["id", "deviceToken"], }, });
    let userData = await user.findOne({ where: { id: req.user.id } });
    if (orderData) {
      ...
      orderData.save().then(async (orderData) => {
        ...
        let charge = await orderCharge.findOne({where:{orderId: orderId}});
        if(charge){
            let zoneRest = await zoneRestaurants.findOne({where:{restaurantId : orderData?.restaurantId},include:{model:zone,attributes:['id'],include:{model:zoneDetails}}});
            let type = await userType.findOne({where:{name:"Admin"}});
            let admin = await user.findOne({where:{userTypeId : type.id}});
            let earning = await driverEarning.findOne({where:{userId : admin.id}});
            let adminEarning = 0;
            let restEarning = 0;

            //admin earning = subTotalCommision + ServiceCharge + Discount + deliveryFeeCommision
            ...
            // rest earning = SubTotal - adminCommision on SubTotal + Packing Fee + VAT + restaurantDelvieryCharges   
            if(isCredit){
                restEarning =  (parseFloat(orderData?.subTotal) - parseFloat(orderData.subTotal) * (zoneRest?.zone?.zoneDetail?.adminComission/100)) + 
                            ...
                            //   parseFloat(charge?.restaurantDeliveryCharges ? charge?.restaurantDeliveryCharges : 0);
                 let userCredit = await Credit.findOne({where:{userId : orderData.userId}});
                 if(userCredit){
                     userCredit.point = 0;
                     await userCredit.save();
                 }
...
async function cancelOrderFood(req, res) {
  ...
  singleNotification(
    formatTokens(orderData?.restaurant?.user?.deviceToken),
    "Order Cancel",
...
async function addRatingFeedback(req, res) {
  ...
  if (deliveryType === "Self-Pickup") {
    ...
    rating.value = parseFloat(driverRate);
    rating.at = cTime;
...
async function orderHistoryRes(req, res) {
  ...
  let cancelledStatus = await orderStatus.findOne({where:{name:'Cancelled'}});
  ...
  where: {
    ...
    userId: userId,
    orderStatusId: {
      [Op.or]: [deliveredStatus.id, cancelledStatus.id]
    }
  },
  order: [["id", "DESC"]],
  include: [
...
const ongoingOrders = await order.findAll({
  where: {
    ...
    orderStatusId: {
      [Op.notIn]: [deliveredStatus.id, cancelledStatus.id]
    }
  },
  order: [["id", "DESC"]],
  include: [
...
const codOrders = await order.findAll({
  ...
  codOrders.map((o, i) => {
    ...
    codOrdersList.push(outObj);
  });
// return res.json(ongoingOrdersList)
  let data = {
    foodOrders,
    ongoingOrdersList,
    codOrdersList
...
//   return res.json(estTime)
  const result = estTime.match(/\d+/)[0];
...
async function placeGroupOrder(req, res) {
  ...
  let method = await paymentMethod.findOne({where:{name : "COD"}});
  ...
  const orderData = await order.findOne({
    ...
    },{model:orderGroup,attributes:['id'],include:{model:user,as:"participant",attributes:['deviceToken']}}],
  });
  let deviceTokens = [];
  ...
  if (orderData) {
    ...
    orderData
      ...
      .then(async (dat) => {
        ...
        // deviceTokens of all Participants
        orderData.orderGroups.forEach(group => {
            const tokens = JSON.parse(group.participant.deviceToken);
            deviceTokens = deviceTokens.concat(tokens);
        });
        //   return res.json(dd)
        let data = {
          orderId: dd.orderId,
        ...
        sendNotifications(deviceTokens, {title:"Group Order",body:"Group Order Placed"});
  ...
  try {
    ...
    retOrderDataForJoinGroup(orderData.id, req.user.id, subTotal).then(
      async (retData) => {
        ...
        let groupMembers = await orderGroup.findAll({where:{orderId : orderId},attributes:[],include:{model:user , as:"participant",attributes:['ip']}});
        var ips = [];
        ...
        sendEvent(JSON.stringify(ips), eventData);
...
async function lockGroupOrder(req,res){
    const { orderId , isLocked } = req.body;
    const orderData = await order.findOne({where:{id:orderId}});
    if(orderData){
        orderData.isLocked = isLocked;
        orderData.save().then(dat =>{
            let response = ApiResponse("1",`Group ${isLocked ? 'Locked' : 'Unlocked'} successfully`,"",{});
            return res.json(response);
        })
        .catch((error)=>{
            let response = ApiResponse("0",error.message,"Error",{});
...
async function removeMember(req, res) {
    ...
    try {
        const group = await orderGroup.findOne({ where: { participantId: userId, orderId: orderId } });
        ...
        const items = await orderItems.findAll({ where: { orderId: group.orderId, userId: userId } });

        for (const item of items) {
            const adds = await orderAddOns.findAll({ where: { orderItemId: item.id } });

            for (const add of adds) {
                await add.destroy();
            }

            await item.destroy(); // Fix the typo from destory to destroy
        }

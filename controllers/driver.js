...
async function driverOnline(req,res){
   try{
        ...
        let driver = await user.findOne({where:{id:driverId}});
        ...
        if (totalOnlineTime >= 4) {
            let response = ApiResponse("0","Driver has already reached max hours!","",{});
            return res.json(response);
        }
        else{
...
async function verifyOTP(req, res) {
  try {
    ...
    if (OTP == "1234" || OTP == otpData?.OTP) {
      ...
      const upData = await user.findOne({ where: { id: existUser.id } });
      ...
      return res.json(output);
        ...
        let response = ApiResponse("0","OTP is invalid","",{});
        return res.json(response);
    }
  } catch (error) {
    const response = ApiResponse(
    ...
    .then((data) => {
      ...
      order
        ...
        .then((orderData) => {
          ...
          sendNotification(to, notification, data).then(async(dat) => {
...
async function delivered(req, res) {
  ...
  let paymentType = await paymentMethod.findOne({where:{name:"Payrexx"}});
